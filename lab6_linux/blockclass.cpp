...
// checks if coordinate is contained in the block
bool block::checkContained(int x,int y) {
	if(x < block_xmax && x > block_xmin && y < block_ymax && y > block_ymin){
		if(1) { // contained in array???
			return true;
		}
	}
	else {
